...
-----------------------------  Mutex  ---------------------------------
  ...
  behavior Mutex
    ...
    --Lock()
        ...
        --         wake up the oldest oneand give it the lock.
        --Init()
      ...
      method Lock ()
          ...
          if(heldBy == currentThread)
              FatalError("Has Already Locked")
          endIf
          while(heldBy != currentThread)
              oldstate = SetInterruptsTo(DISABLED) --获取执行前的中断状态，并禁止中断发生
                  if heldBy == null
      ...
      method Unlock ()
          ...
          if (heldBy != currentThread)
              FatalError("No thread hold mutex lock")
          endIf
          heldBy = null
