...
-----------------------------  Dining Philosophers  ---------------------------------
  ...
  behavior ForkMonitor
    ...
    method Init ()
      -- Initialize so that all philosophers are THINKING.
        status = new array of int{ 5 of THINKING }
          s = new array[5] of Semaphore{ 5 of new Semaphore }
      -- ...unimplemented...
        monMutex = new Semaphore
        monMutex.Init(1)
          ...
          for ii = 0 to 4
              s[ii] = new Semaphore
              s[ii].Init(0)  --Initialize each semaphore with an initial count of 0
              endFor
    ...
    method test (p: int)
        if status[p] == HUNGRY && status[(p+5-1)%5]!=EATING && status[(p+1)%5]!=EATING
            status[p] = EATING
            s[p].Up()
        endIf
        self.PrintAllStatus()
